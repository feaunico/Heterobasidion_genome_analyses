...
ccs = []
with open("5396_ccs.fastq") as infile:
    for line in infile:
...
with open("5396_long.fastq") as infile:
    for line in infile:
        if line[0] in ['A','C','G','T','N']:
...
with open("5396_sub.fastq") as infile:
    for line in infile:
        if line[0] in ['A','C','G','T','N']:
            sub.append(len(line))
print 'done 2'
...
ax0.plot((0.5,0.5),(0000,115000),'k-',linewidth=1.5,clip_on=False)
...
ax0.text(0.,115000,'C',fontsize=18)

ax2.text(-11000,140000,'D',fontsize=18)

plt.savefig('5396.png',dpi=600,  format='png')
